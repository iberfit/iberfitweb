...
js = r'''(()=>{"use strict";const enhance=()=>{document.querySelectorAll('.answer-section').forEach(section=>{if(section.dataset.aeoV6437==='1')return;const list=section.querySelector('.answer-list');if(!list)return;const items=[...list.querySelectorAll(':scope > .answer-item')];if(!items.length)return;let made=0;items.forEach((item,index)=>{const question=item.querySelector('h2,h3,h4');if(!question)return;const details=document.createElement('details');details.className='answer-disclosure-v6437';const summary=document.createElement('summary');const number=document.createElement('span');number.className='answer-index-v6437';number.textContent=String(index+1).padStart(2,'0');const label=document.createElement('span');label.className='answer-question-v6437';label.textContent=question.textContent.trim();const toggle=document.createElement('span');toggle.className='answer-toggle-v6437';toggle.setAttribute('aria-hidden','true');summary.append(number,label,toggle);const body=document.createElement('div');body.className='answer-body-v6437';[...item.childNodes].forEach(node=>{if(node!==question)body.appendChild(node)});details.append(summary,body);details.addEventListener('toggle',()=>{if(details.open){list.querySelectorAll('details.answer-disclosure-v6437[open]').forEach(other=>{if(other!==details)other.open=false})}});item.replaceWith(details);made++});if(made){section.classList.add('aeo-disclosure-v6437');section.dataset.aeoV6437='1'}})};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',enhance,{once:true}):enhance()})();
'''
